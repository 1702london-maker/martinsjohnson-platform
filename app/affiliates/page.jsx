...
export default function AffiliatesPage() {
  const [form, setForm] = useState({ name:'', email:'', phone:'', platform:'', handle:'', audience_size:'', tier:'creator', location:'', why:'' })
  ...
  async function submit(e) {
    ...
    try {
      ...
      setSent(true)
      toast.success('Application received. Our team will be in touch within 48 hours.')
    } catch (err) {
      ...
      <div className="bg-mj-dk1 border-b border-mj-b1 px-6 md:px-14 lg:px-20 pt-20 pb-24 relative overflow-hidden">
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:.8}} className="relative z-10">
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            <Link href="/affiliates/dashboard" className="inline-flex items-center gap-2 px-7 py-3.5 border border-white/20 text-white/50 text-[10px] tracking-[0.22em] uppercase font-medium hover:border-white/40 hover:text-white/80 transition-all">Affiliate Dashboard →</Link>
          </div>
        </motion.div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 border border-mj-b1">
          {TIERS.map(tier=>(
            <div key={tier.id} className="border-b md:border-b-0 md:border-r border-mj-b1 last:border-0 p-8 hover:bg-mj-bg3 transition-colors">
              ...
              <p className="text-[10px] tracking-widest uppercase text-mj-t5 mb-5 font-medium">Commission &middot; {tier.followers}</p>
      ...
      <div id="apply" className="px-6 md:px-14 lg:px-20 py-16">
        ...
        {sent ? (
          <div className="max-w-lg border border-mj-b1 p-12 text-center">
            ...
            <p className="text-[14px] text-mj-t4 font-light mb-6">Our team reviews all applications within 48 hours. We will be in touch at the email you provided.</p>
            <Link href="/affiliates/dashboard" className="btn-solid">View Dashboard →</Link>
          </div>
        ) : (
          <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl">
            {[
              ...
              { id:'phone',         label:'Phone Number',         type:'text',  placeholder:'+44 7000 000000' },
              ...
              { id:'audience_size', label:'Audience Size',        type:'text',  placeholder:'e.g. 50,000' },
            ...
            <div className="md:col-span-2">
              ...
              <div className="border-b border-mj-b1 focus-within:border-mj-t1 transition-colors">
                <textarea value={form.why} onChange={e=>setForm(p=>({...p,why:e.target.value}))} rows={4}
